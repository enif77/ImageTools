...
using ImageTools.Core.PixelInterpolators;
...
internal class RenderState
{
    ...
    public ImageData Image { get; set; } = new ImageData(1, 1);
}
...
public class EquirectangularProjectionToCubeMapConverter
{
    ...
    public ImageData RenderFace(ImageData sourceImage, CubeMapFaceOrientation cubeMapFaceOrientation, double rotation, ImageInterpolationStrategy interpolationStrategy, int maxWidth = int.MaxValue, int maxThreads = 1)
    {
        ...
        var destinationImage = new ImageData(faceSize, faceSize, 4);
        ...
        for (var top = 0; top < faceSize; top += subFaceSize)
        {
            for (var left = 0; left < faceSize; left += subFaceSize)
            {
                var subFaceState = new RenderState()
                {
                    Left = left,
                    Top = top,
                    Image = new ImageData(subFaceSize, subFaceSize, 4)
                };
    ...
    private static void BlitImage(RenderState source, ImageData destination)
    {
